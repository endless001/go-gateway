...
// The ServiceInfoFunc type is an adapter to allow the use of ordinary
// function as ServiceInfo mutator.
type ServiceInfoFunc func(context.Context, *ent.ServiceInfoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ServiceInfoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ServiceInfoMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ServiceInfoMutation", m)
	}
	return f(ctx, mv)
}
